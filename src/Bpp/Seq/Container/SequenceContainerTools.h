...
public:
   ...
   */
  template<class SequenceTypeI, class HashTypeI, class SequenceTypeO, class HashTypeO>
  static void getSelectedSequences(
      const TemplateSequenceContainerInterface<SequenceTypeI, HashTypeI>& sequences,
      const SequenceSelection& selection,
      TemplateSequenceContainerInterface<SequenceTypeO, HashTypeO>& outputCont)
  {
    for (size_t position : selection)
    {
      auto seq = std::make_unique<SequenceTypeO>(sequences.sequence(position));
      ...
      bool strict = true)
  {
    for (const std::string& key : selection)
    {
      if (strict)
      {
        auto seq = std::make_unique<SequenceTypeO>(sequences.sequence(key));
        outputCont.addSequence(seq->getName(), seq);
      }
      else
      {
